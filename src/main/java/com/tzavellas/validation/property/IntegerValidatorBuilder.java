...
import com.tzavellas.validation.validators.simple.IntegerValidators;

/**
 * A property validator for {@link Integer} and {@link Long} objects.
 * 
 ...
 * @see Validators
 */
public class IntegerValidatorBuilder extends LinkedValidatorBuilder<Number, IntegerValidatorBuilder> {

	/**
	 * Create a <code>IntegerValidatorBuilder</code> for the specified
	 * property name.
	 * 
	 * <p>Usually instances of this class are created with the
	 * {@link Validators#integer(String)} factory method.
	 * 
	 ...
	 * @see Validators
	 */
	public IntegerValidatorBuilder(String property) {
		super(property);
	}
	 ...
	 */
	public IntegerValidatorBuilder min(final long min) {
		addValidator(new PropertyValidator.Builder<Number>(property)
			.withErrorCode("validator.integer.min")
			.withMessageArgs(min)
			.withDefaultMessage("Must be greater than " + min)
			.withValidator(new IntegerValidators.Min(min)).build());
		return this;
	}
	 ...
	 * specified <code>long</code>.
	 */
	public IntegerValidatorBuilder max(final long max) {
		addValidator(new PropertyValidator.Builder<Number>(property)
			.withErrorCode("validator.integer.max")
			.withMessageArgs(max)
			.withDefaultMessage("Must be less than " + max)
			.withValidator(new IntegerValidators.Max(max)).build());
		return this;
	}
	 ...
	 */
	public IntegerValidatorBuilder range(final long min, final long max) {
		addValidator(new PropertyValidator.Builder<Number>(property)
			.withErrorCode("validator.integer.range")
			.withMessageArgs(min, max)
			.withDefaultMessage("Must be greater than or " + min + " and less than " + max)
			.withValidator(new IntegerValidators.Range(max, min)).build());
		return this;
	}
}
